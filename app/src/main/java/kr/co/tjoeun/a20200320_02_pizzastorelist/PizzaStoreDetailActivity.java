...
public class PizzaStoreDetailActivity extends BaseActivity {
    ...
    public void setupEvents() {
        ...
        binding.storeLogo.setOnClickListener(new View.OnClickListener() {
            @Override
...
//                          허가가 떨어진 상황 => 전화걸기 코드 실행
                        ...
                        Uri uri = Uri.parse(phoneNumUri); // 띄어쓰기 or - 들어가지 않게.
...
//                          전화 권한을 허가 해야 사용 가능합니다 Toast
                        Toast.makeText(mContext, "전화 권한을 허가 해야 사용 가능합니다.", Toast.LENGTH_SHORT).show();
                    }
                };

//                onClick 실행? 사용자가 손으로 클락할 때 실행
//                권한 상황 대응 실행? 코드로 권한 요청을 직접 날려서 실행
    ...
    public void setValues() {
        ...
        Glide.with(mContext).load(store.getLogoUrl()).into(binding.storeLogo);
